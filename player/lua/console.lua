...
function format_table(list, width_max, rows_max)
    ...
    local spacing = string.format('%' .. spaces .. 's', ' ')
    ...
    for _, completer in ipairs(build_completers()) do
        ...
        if s then
            ...
            if #completions > 0 then
                ...
                if #completions == 1 and completer.append then
                    prefix = prefix .. completer.append
                else
                    table.sort(completions)
                    suggestion_buffer = {}
                    for _, completion in ipairs(completions) do
                        suggestion_buffer[#suggestion_buffer + 1] = completion
                    end
                end
